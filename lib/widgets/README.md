...
- [session_preview_sheet.dart](session_preview_sheet.dart): Bottom sheet renderer for session previews, composed of header + exercises list.
...
- Exposed widgets/classes: `SessionPreviewSheet`, `SessionHeaderCard`, `SessionExercisesList`, `SessionExerciseCard`, `ProgressLineChart`, `ProgressFilters`, `ProgressPointsRecap`, `showCreateExerciseDialog`.
- Navigation: Sheet utilities may call `Navigator.pop`; preview sheet triggered from Library/Home to deep link toward `/sessions/:id`.
- Events/commands: Dialog returns `CreatedExercise`; cards expose expansion toggles; filters emit callbacks on chip selection.
...
**Gotchas & Conventions**
- `SessionPreviewSheet` expects a `Future<SessionDetail>`; handle errors upstream or rely on built-in error text.
