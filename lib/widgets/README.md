...
- Exposed widgets/classes: `SessionPreviewSheet`, `SessionPreviewAction`, `SessionDetailBody`, `SessionPrimaryActionButton`, `SessionHeaderCard`, `SessionExercisesList`, `SessionExerciseCard`, `ProgressLineChart`, `ProgressFilters`, `ProgressPointsRecap`, `showCreateExerciseDialog`.
- Navigation: Sheet utilities may call `Navigator.pop`; preview sheet triggered from Library/Home to deep link toward `/sessions/:id` or `/log` with edit extras.
- Events/commands: Dialog returns `CreatedExercise`; preview sheets optionally surface `SessionPreviewAction` callbacks; cards expose expansion toggles; filters emit callbacks on chip selection.
