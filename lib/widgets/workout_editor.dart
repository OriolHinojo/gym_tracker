...
typedef WorkoutEditorSaveCallback = FutureOr<void> Function(
  ...
  WorkoutEditorResult result,
);

/// Result describing the outcome of a workout edit/save operation.
class WorkoutEditorResult {
  WorkoutEditorResult._(this.workoutId, this.isUpdate);

  factory WorkoutEditorResult.saved(int workoutId) => WorkoutEditorResult._(workoutId, false);

  factory WorkoutEditorResult.updated(int workoutId) => WorkoutEditorResult._(workoutId, true);

  final int workoutId;
  final bool isUpdate;

  bool get isNew => !isUpdate;
}

/// Shared editor used by the log screen and inline editing flows.
class WorkoutEditor extends StatefulWidget {
  ...
  final WorkoutEditorSaveCallback? onSaved;
  final void Function(BuildContext context)? onDiscarded;

  @override
  State<WorkoutEditor> createState() => _WorkoutEditorState();
}

/// Pushes the shared editor as a full-screen page from any context.
Future<void> showWorkoutEditorPage(
  BuildContext context, {
  int? templateId,
  String? workoutId,
  int? editWorkoutId,
  WorkoutEditorSaveCallback? onSaved,
  void Function(BuildContext context)? onDiscarded,
}) {
  final rootContext = context;
  return Navigator.of(context).push(
    MaterialPageRoute(
      builder: (routeCtx) => WorkoutEditor(
        templateId: templateId,
        workoutId: workoutId,
        editWorkoutId: editWorkoutId,
        onSaved: (editorContext, result) async {
          if (onSaved != null) {
            await onSaved(rootContext, result);
            return;
          }
          Navigator.of(editorContext).maybePop();
          if (result.isUpdate) {
            rootContext.push('/sessions/${result.workoutId}');
          }
        },
        onDiscarded: (editorContext) {
          if (onDiscarded != null) {
            onDiscarded(rootContext);
          } else {
            Navigator.of(editorContext).maybePop();
          }
        },
...
class _WorkoutEditorState extends State<WorkoutEditor> {
  ...
  _HistoryComputationResult _computeHistoryFromRaw(List<Map<String, dynamic>> rawSets) {
    ...
    for (final set in sorted) {
      ...
      final effectiveOrdinal =
          ordinal ?? (historyEntries.isEmpty ? 1 : historyEntries.last.ordinal + 1);
  ...
  Future<void> _finishAndPersist() async {
    ...
    if (_editingWorkoutId != null) {
      ...
      if (callback != null) {
        await callback(context, WorkoutEditorResult.updated(editingId));
      } else {
        Navigator.of(context).maybePop();
      }
      return;
    ...
    final newId = await LocalStore.instance.saveWorkout(
      userId: 1,
      name: 'Workout',
      sets: sets,
      templateId: _activeTemplateId,
    ...
    final callback = widget.onSaved;
    if (callback != null) {
      await callback(context, WorkoutEditorResult.saved(newId));
    } else {
      Navigator.of(context).maybePop();
    }
  }
